...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        self._neos_list = list()
        self._approaches_list = list()

        # Link together the NEOs and their close approaches.
        for key, neo in self._neos.items():
            approaches_ = self._approaches.get(neo.designation, None)
            if approaches_ is not None:
    ...
    def get_approaches_list(self):
        if len(self._approaches_list) <= 0:
            for des, apps in self._approaches.items():
                for app in apps:
                    self._approaches_list.append(app)
        return self._approaches_list
    ...
    def query(self, filters=()):
        ...
        for approach in self.get_approaches_list():
            if all([apply_filter(approach) for apply_filter in filters]):
                yield approach
